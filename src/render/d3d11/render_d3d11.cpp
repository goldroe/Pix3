...
internal void d3d11_render(OS_Handle window_handle, Draw_Bucket *draw_bucket) {
    ...
    float clear_color[4] = {.12f, .12f, .12f, 1.f};
    ...
    for (R_Batch_Node *batch_node = draw_bucket->batches.first; batch_node; batch_node = batch_node->next) {
        ...
        case R_ParamsKind_Quad:
        {
            ...
            r_d3d11_state->device_context->PSSetSamplers(0, 1, &r_d3d11_state->samplers[params_quad->sampler]);
            ...
            // quad_uniform.xform = ortho_rh_zo(draw_region.x0, draw_region.x1, draw_region.y0, draw_region.y1, -1.f, 1.f);
